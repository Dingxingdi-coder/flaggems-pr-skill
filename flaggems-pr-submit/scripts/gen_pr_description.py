...
DOMESTIC_GPU_DIR = Path("/data/dxd/国产GPU算子测试情况")
...
def find_benchmark_file(repo_dir: Path, op_name: str) -> Path | None:
    ...
    op_id = resolve_op(op_name).op_id
    bare = op_name.lstrip("_")
    for name in dict.fromkeys((op_name, op_id, bare)):
        bench_file = repo_dir / "benchmark" / f"test_{name}.py"
        if bench_file.exists():
            return bench_file
    # fallback: glob
    candidates = list(repo_dir.glob(f"benchmark/test_{op_id}*.py"))
    if not candidates and bare != op_id:
...
def _strip_aten_prefix(name: str) -> str:
...
def _normalized_operator_key(name: str) -> str:
    """Normalize operator names while preserving inplace suffix identity."""
    bare = _strip_aten_prefix(name)
...
def _lookup_operator(
    ...
    if try_aten:
        for prefix in ("aten::", "aten__"):
            ...
            if f"{prefix}{op_id}" in ops:
                return ops[f"{prefix}{op_id}"]
            if f"{prefix}{bare}" in ops:
                return ops[f"{prefix}{bare}"]
    # 3. Normalized match: snake_case, CamelCase, and Aten prefixes.
    target_keys = {
        _normalized_operator_key(op_name),
        _normalized_operator_key(op_id),
        _normalized_operator_key(bare),
    }
    for k, v in ops.items():
        if _normalized_operator_key(k) in target_keys:
            return v

    # 4. Fuzzy: key contains op_name
    for k, v in ops.items():
        normalized = k.replace("::", "_").replace("_", " ")
        if op_name in k or op_id in k or bare in normalized:
            return v
    return None
...
def _resolve_log_file(summary_dir: Path, log_file_path: str, op_name: str, key: str) -> str:
    ...
    Summary JSONs may contain original machine paths like
    /path/to/test_results_*/op_test.log — those don't exist locally.
    ...
    for search_dir in search_dirs:
        ...
        op_id = resolve_op(op_name).op_id
        for name in dict.fromkeys((op_name, op_id, op_name.lstrip("_"))):
...
def _get_bench_command(backend_name: str, op_name: str) -> str:
    ...
    op_id = resolve_op(op_name).op_id
    shared_file_map = {
        "tianshu": f"pytest benchmark/test_unary_pointwise_perf.py -m {op_id}",
        "muxi": f"pytest benchmark/test_unary_pointwise_perf.py -m {op_id}",
        "ascend": f"pytest benchmark/test_unary_pointwise_perf.py -m {op_id}",
        "hygon": f"pytest benchmark/test_unary_pointwise_perf.py -m {op_id}",
    }
...
def main():
    ...
    p.add_argument(
        "--repo",
        default="/data/dxd/FlagGems_minimax_2_7_pr",

...
| Review and repair generated worktree first | Soft | `references/implementation-review/spec.md` | `implementation-review` |
...
| Accuracy test correctness, reference conversion, NaN handling, statistical validation | Soft | `references/implementation-review/spec.md` and `references/shared/test-benchmark.md` | `implementation-review`, `worktree-test-benchmark`, `final-validation` |
| Benchmark mark and `op_name` alignment | Hard | `operator_static_gate.py` / `STATIC-H008`, `STATIC-H009` | `register`, `final-validation` |
| Benchmark wrapper fairness, equivalent torch/gems work, nonzero cases, speedup threshold | Soft | `references/shared/test-benchmark.md` and semantic review | `implementation-review`, `worktree-test-benchmark`, `final-validation` |
...
| Exported wrappers are registered, tested, and benchmarked | Soft | `references/register/spec.md` | `register`, `final-validation` |
| Formatting checks, explicit staging, commit message | Soft | `references/final-validation/spec.md` and command output | `final-validation` |
| PR body completeness and truthful performance data | Soft | `references/final-validation/spec.md` | `final-validation` |
| Review follow-up exclusion for initial PR creation | Soft | `SKILL.md` scope | main agent, `final-validation` |
| Reviewer-derived rule intake and promotion | Soft-to-hard workflow | `references/reviewer-feedback-intake.md`, `references/soft-constraints.md`, owning scripts, and canonical specs | maintenance agent |

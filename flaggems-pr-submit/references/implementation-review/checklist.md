...
- Before repairing implementation, tests, or benchmarks, branch on `IS_ATEN`. For `IS_ATEN=true`, confirm the resolved target has an exact `torch.ops.aten` schema. For `IS_ATEN=false`, treat prompt metadata as optional initial context; inspect the target implementation, tests, benchmarks, and YAML to confirm and repair the fused/custom target, public API, and PyTorch reference expression. If the target identity cannot be uniquely determined from those source surfaces, block and report the missing evidence instead of retargeting to the nearest ATen schema automatically.

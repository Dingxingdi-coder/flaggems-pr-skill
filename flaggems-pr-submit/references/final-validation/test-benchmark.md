# Final Validation Test and Benchmark Rules

Applies to: final-validation subagent.

All commands run inside `{GEN_WORKTREE}` on the generated branch with the assigned GPU. These checks are the final gate before staging, commit, push, and PR creation.

...
For special ops: read `references/name-worktree/special-ops-mapping.md` for the mapping from normalized name to module and public op id. Special ops are: `max_pool2d_with_indices_backward`, `matmul_layer_norm`, `__and__`.
...
Check whether the worktree exists. If not, stop.

## Upstream conflict

Fetch upstream first. Check upstream for `src/flag_gems/ops/<module>.py` and yaml `id: <op_id>`. If either exists, stop this operator.
